...
class CityList extends React.Component {
  ...
  changeCity = (city) => {
    ...
    // 判断点击是否是当前城市
    let cityRes = ACTIVE_CITY.indexOf(label)
    // console.log(cityRes)
    if (cityRes > -1) {
      // 存在就存储到本地
      localStorage.setItem('hkzf_my', JSON.stringify({ label, value }))
      // 返回到上一页
      this.props.history.go(-1)
    } else {
      // 提示用户不存在
      Toast.offline('你找的信息不存在,请稍后再试...', 2)
    }
  }
  ...
  cityHeight = ({ index }) => {
    ...
    let list = citylist[letter]  //每个字表的数组
    // 算出高度
  ...
  onRowsRendered = ({ startIndex, stopIndex }) => {
    ...
    this.setState({
      selectedLetter: startIndex  //可视区域展示的第一行的下标
    })
  }
  // List组件中要调用的函数
  ...
  render() {
    let { cityIndex, selectedLetter } = this.state
    return (
        ...
        <AutoSizer>
          {({ width, height }) => (
            <List
              ...
              rowRenderer={this.rowRenderer}  //组件渲染
